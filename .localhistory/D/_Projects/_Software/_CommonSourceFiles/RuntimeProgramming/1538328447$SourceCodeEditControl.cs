...
*/
        ...
        public void SelectCharAtPos(int linePos, int colPos)
        {
            try
            {
                ...
                fastColoredTextBox.DoSelectionVisible(); // scroll to selection
            }
            catch (Exception ex)
            {
		...
		private void fastColoredTextBox_setRightClickContextMenu()
        {
            ...
            menuItem.Click += new EventHandler(IncreaseLineIndent_Action);
            contextMenu.MenuItems.Add(menuItem);
            menuItem = new MenuItem("Decrease Line Indent");
            menuItem.Click += new EventHandler(DecreaseLineIndent_Action);
            contextMenu.MenuItems.Add(menuItem);
        ...
        void IncreaseLineIndent_Action(object s, EventArgs ea)
        {
            fastColoredTextBox.IncreaseIndent();
        }
        void DecreaseLineIndent_Action(object s, EventArgs ea)
        {
            ...
            for (int i = 0; i < sourceFilesRef.Count; i++)
            {
                if (sourceFilesRef[i].FileName.ToLower() == fileName)
                {
                    ...
                    if (fastColoredTextBox.Tag != null)
                    {
                        ...
                        //sf.editorSelectionRange = fastColoredTextBox.Selection;
                        sf.editorSelectionStart = fastColoredTextBox.SelectionStart;
                        sf.editorSelectionLength = fastColoredTextBox.SelectionLength;

                        sf.editorVerticalScrollValue = fastColoredTextBox.VerticalScroll.Value;
                        sf.editorHorizontalScrollValue = fastColoredTextBox.HorizontalScroll.Value;
                        
                    }
                    
                    fastColoredTextBox.Text = sourceFilesRef[i].Contents;
                    fastColoredTextBox.Tag = sourceFilesRef[i];
                    fastColoredTextBox.SelectionStart = sourceFilesRef[i].editorSelectionStart;
                    fastColoredTextBox.SelectionLength = sourceFilesRef[i].editorSelectionLength;
                    /*
                    if (sourceFilesRef[i].editorSelectionRange != null)
                    {
                        fastColoredTextBox.
                        fastColoredTextBox.Selection = sourceFilesRef[i].editorSelectionRange;
                        fastColoredTextBox.Update();
                        //fastColoredTextBox.DoSelectionVisible();
                    }
                    else
                        rtxtLog.AppendText("ok @ sourceFilesRef[i].editorSelectionRange == null\r\n");
                    */
                    fastColoredTextBox.HorizontalScroll.Value = sourceFilesRef[i].editorHorizontalScrollValue;
                    fastColoredTextBox.VerticalScroll.Value = sourceFilesRef[i].editorVerticalScrollValue;
                    fastColoredTextBox.UpdateScrollbars();
